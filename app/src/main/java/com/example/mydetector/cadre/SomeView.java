...
public class SomeView extends View implements View.OnTouchListener {
    private Paint paint;
    public static List<Coordonnées> points;
    int DIST = 2;
    boolean flgPathDraw = true;

    Coordonnées mfirstpoint = null;
    boolean bfirstpoint = false;

    Coordonnées mlastpoint = null;
    int countPoints = 0;
    ...
    public SomeView(Context c) {
        ...
        points = new ArrayList<Coordonnées>();
        coords = new ArrayList<Coordonnées>();
        bfirstpoint = false;
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < points.size(); i += 2) {
            Coordonnées point = points.get(i);
            if (first) {
                first = false;
                path.moveTo(point.x, point.y);
            } else if (i < points.size() - 1) {
                Coordonnées next = points.get(i + 1);
    ...
    public boolean onTouch(View view, MotionEvent event) {
        ...
        Coordonnées point = new Coordonnées();
        ...
        if (countPoints % 2 == 0) {
            coords.add(new Coordonnées(point.x, point.y));
        }
        countPoints++;
    ...
    private boolean comparepoint(Coordonnées first, Coordonnées current) {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                switch (which) {
                    case DialogInterface.BUTTON_POSITIVE:
                        ...
                        listItem.getInstance().setCoords(coords);
